...
brick2 = Entity(model="cube", color=color.yellow, texture="brick", scale=(1, 0.5, 0.5), position=(-4.5, 2),
                collider="box")
...
brick = Entity(model="cube", color=color.yellow, texture="brick", scale=(1, 0.5, 0.5), position=(-5.5, 1),
               collider="box")
scoring.append(brick)
...
hitter = Entity(model="cube", texture="brick", scale=(2, 0.5, 0.5), position=(0, -3), collider="box")
destroy1.append(hitter)
surface = []

surface_top = Entity(model="cube", texture="brick", scale=(1, 0.5, 0.5), position=(-7, 4), collider="box")
for i in range(-7, +8):
    walls.append(duplicate(surface_top, x=i))
surface2 = []
surface_left = Entity(model="cube", texture="brick", scale=(0.5, 1, 0.5), position=(-7, 4), collider="box")
for i in range(-7, +8):
    surface2.append(duplicate(surface_left, y=i))

surface_right = Entity(model="cube", texture="brick", scale=(0.5, 1, 0.5), position=(7, 4), collider="box")
for i in range(-7, +8):
